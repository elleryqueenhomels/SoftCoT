...
def judge_answer(input, label, data_name="gsm8k", extract=True, model_name="Qwen2.5-7B-Instruct"):
    """Score.
    ...
    if "gsm8k" in data_name.lower() or "asdiv-aug" in data_name.lower():
        if extract:
            input = extract_answer(input, data_name="gsm8k", model_name=model_name)
        return (input == label)
    elif "math-500" in data_name.lower():
        if extract:
            input = extract_answer(input, data_name="math-500", model_name=model_name)
        ...
        if extract:
            input = extract_answer(input, data_name="aime", model_name=model_name)
            input = str(input)
            label = str(label)
        return (input == label)
...
def extract_answer(text, data_name="gsm8k", model_name="Qwen2.5-7B-Instruct"):
    '''
